...
Profiles are **completely separate** from simple mode coloring:

- When a rule uses a profile name (e.g., `repo:ProfileName`), simple mode `repoColor` and `branchColor` settings are **ignored**
- The profile's palette sources named "repoColor" and "branchColor" are internal palette slots within the profile system
- These palette sources are NOT references to simple mode colors—they are just names for palette slot sources
- Profiles provide a complete, self-contained color system that replaces simple mode logic
